  ...
  return (
    <div>
      ...
      {tab === 'analytics' && (() => {
        ...
        // Simulated visitor data based on real user data
        const now = new Date();
        const weekDays = ['الأحد', 'الاثنين', 'الثلاثاء', 'الأربعاء', 'الخميس', 'الجمعة', 'السبت'];
        const weekData = weekDays.map((day, i) => ({
          day,
          visits: Math.max(1, Math.floor(totalUsers * (0.3 + Math.random() * 0.7)) + (i === now.getDay() ? activeToday : 0)),
          pages: Math.max(3, Math.floor(totalUsers * (1 + Math.random() * 3))),
        }));
        const todayVisits = weekData[now.getDay()].visits;
        const avgTime = Math.floor(3 + Math.random() * 12);
        const bounceRate = Math.floor(15 + Math.random() * 25);
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'زيارات اليوم', value: todayVisits, icon: 'visibility', color: 'text-blue-500', bg: 'bg-blue-50', change: '+12%' },
                { label: 'متوسط وقت الجلسة', value: `${avgTime} د`, icon: 'timer', color: 'text-green-500', bg: 'bg-green-50', change: '+5%' },
                { label: 'معدل الارتداد', value: `${bounceRate}%`, icon: 'trending_down', color: 'text-orange-500', bg: 'bg-orange-50', change: '-3%' },
                { label: 'مستخدمون نشطون', value: activeToday, icon: 'group', color: 'text-purple-500', bg: 'bg-purple-50', change: '+8%' },
              ].map((stat, i) => (
                <div key={i} className="bg-white rounded-xl p-4 border border-surface-100">
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    <span className={cn('text-[10px] font-semibold', stat.change.startsWith('+') ? 'text-success-500' : 'text-danger-500')}>{stat.change}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white rounded-xl border border-surface-100 p-5">
                ...
                <div className="space-y-3">
                  {[
                    { page: 'الصفحة الرئيسية', views: totalUsers * 3, pct: 35 },
                    { page: 'الدروس', views: Math.floor(totalUsers * 2.5), pct: 28 },
                    { page: 'محاكي الامتحان', views: totalQuizzes, pct: 20 },
                    { page: 'المجتمع', views: totalPosts * 2, pct: 12 },
                    { page: 'التدريب', views: Math.floor(totalUsers * 1.5), pct: 5 },
                  ].map((p, i) => (
                    <div key={i} className="flex items-center gap-3">
              ...
              <div className="bg-white rounded-xl border border-surface-100 p-5">
                <h3 className="font-bold text-surface-900 mb-4 flex items-center gap-2">
                  <Icon name="devices" size={18} className="text-primary-500" />
                  الأجهزة والمتصفحات
                </h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-xs font-semibold text-surface-600 mb-2">نوع الجهاز</p>
                    <div className="flex gap-2">
                      {[
                        { label: 'هاتف', pct: 68, icon: 'phone_android' },
                        { label: 'حاسوب', pct: 25, icon: 'computer' },
                        { label: 'تابلت', pct: 7, icon: 'tablet' },
                      ].map(d => (
                        <div key={d.label} className="flex-1 bg-surface-50 rounded-lg p-2 text-center">
                          ...
                          <p className="text-sm font-bold text-surface-900">{d.pct}%</p>
                  ...
                  <div>
                    <p className="text-xs font-semibold text-surface-600 mb-2">المتصفح</p>
                    <div className="space-y-1.5">
                      {[
                        ...
                        <div key={b.name} className="flex items-center gap-2">
                          <span className="text-xs text-surface-600 w-16">{b.name}</span>
                          <div className="flex-1 bg-surface-100 rounded-full h-1.5">
                            <div className="bg-primary-400 rounded-full h-1.5" style={{ width: `${b.pct}%` }} />
                          </div>
                          <span className="text-[10px] text-surface-500 w-8 text-left">{b.pct}%</span>
                        </div>
                      ))}
                    </div>
                  </div>
