...
function VerifiedBadge() {
  return <svg className="w-3.5 h-3.5 text-blue-500 shrink-0 inline-block" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/><circle cx="12" cy="12" r="11" fill="none" stroke="currentColor" strokeWidth="2"/></svg>;
}
...
export function CommunityPage() {
  ...
  const [viewUserData, setViewUserData] = useState<{ name: string; avatar: string; bio: string; verified: boolean; postsCount: number; followersCount: number; hideStats: boolean } | null>(null);
  ...
  // Load verified users map, following list, and quiz votes
  useEffect(() => {
    if (user) {
      const storedFollowing = localStorage.getItem(`following_${user.id}`);
      if (storedFollowing) setFollowing(JSON.parse(storedFollowing));
      const storedVotes = localStorage.getItem(`quizVotes_${user.id}`);
      if (storedVotes) {
        const parsed = JSON.parse(storedVotes);
        setQuizVoted(parsed.voted || {});
        setQuizSelected(parsed.selected || {});
      }
    }
  ...
  const toggleFollow = (userId: string) => {
    ...
    localStorage.setItem(`following_${user.id}`, JSON.stringify(newFollowing));
  };
      ...
      <div className={cn('bg-white rounded-2xl border overflow-hidden', post.pinned ? 'border-amber-200 ring-1 ring-amber-100' : 'border-surface-100')}>
        ...
        <div className="p-5">
          ...
          {editingPost === post.id ? (
            ...
            <>{post.content && <p className="text-surface-700 text-sm leading-relaxed whitespace-pre-wrap mb-3">{post.content}</p>}</>
          )}
  ...
  // Post detail view - NO duplicate comments section
  if (detailPostId) {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {sortedPosts.length === 0 ? (
        ...
        <div className="space-y-4">
          {sortedPosts.map(post => <div key={post.id}>{renderPost(post)}</div>)}
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {viewUserData.hideStats ? (
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                ...
                <div className="bg-surface-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-[10px] text-surface-500">متابع</p>
                </div>
                <div className="bg-surface-50 rounded-xl p-3 text-center">
            ...
            <div className="mt-4 max-h-48 overflow-y-auto space-y-2">
              ...
              {posts.filter(p => p.userId === viewUserId).slice(0, 10).map(p => (
                <div key={p.id} className="bg-surface-50 rounded-lg p-2">
