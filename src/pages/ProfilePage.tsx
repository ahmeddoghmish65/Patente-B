  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-2xl p-6 border border-surface-100">
        <div className="flex items-start gap-4 mb-5">
          <div className="relative group">
            ...
            {user.avatar && (
              <button className="absolute -top-1 -left-1 w-6 h-6 bg-danger-500 rounded-full flex items-center justify-center border-2 border-white shadow-sm hover:bg-danger-600 transition-colors"
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              ...
              {user.verified && <Icon name="verified" size={18} className="text-primary-500" filled />}
            </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-surface-100">
        ...
        {!user.profileComplete && (
          <button className="mt-3 w-full bg-warning-50 text-warning-700 rounded-lg py-2.5 text-sm font-medium border border-warning-200 hover:bg-warning-100 transition-colors" onClick={() => setShowCompleteProfile(true)}>
      ...
      {showEditOverlay && (
        <div className="fixed inset-0 z-50 bg-white overflow-y-auto">
          <div className="max-w-lg mx-auto p-4 pb-20">
            {/* Header */}
            <div className="flex items-center justify-between mb-6 sticky top-0 bg-white py-3 border-b border-surface-100 -mx-4 px-4">
              <button onClick={() => setShowEditOverlay(false)} className="flex items-center gap-2 text-surface-500 hover:text-primary-600">
            ...
            {/* Personal Info */}
            <div className="bg-surface-50 rounded-2xl p-5 mb-5">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white" value={editForm.firstName} onChange={e => setEditForm(f => ({ ...f, firstName: e.target.value }))} />
                  </div>
                  <div>
                    <label className="text-xs text-surface-500 mb-1 block">اسم العائلة</label>
                    <input className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white" value={editForm.lastName} onChange={e => setEditForm(f => ({ ...f, lastName: e.target.value }))} />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">اسم المستخدم</label>
                  <input className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white" dir="ltr" value={editForm.username} onChange={e => setEditForm(f => ({ ...f, username: e.target.value }))} placeholder="username" />
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">نبذة عني</label>
                  <textarea className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white resize-none" rows={2} value={editForm.bio} onChange={e => setEditForm(f => ({ ...f, bio: e.target.value }))} maxLength={150} placeholder="اكتب نبذة عنك..." />
                ...
                <div>
                  ...
                  <input type="date" className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white" value={editForm.birthDate} onChange={e => setEditForm(f => ({ ...f, birthDate: e.target.value }))} />
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[{ value: 'male', label: 'ذكر ♂️' }, { value: 'female', label: 'أنثى ♀️' }].map(g => (
                      <button key={g.value} className={cn('py-2.5 rounded-xl border-2 text-sm font-medium', editForm.gender === g.value ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-surface-200 text-surface-600 bg-white')}
            ...
            <div className="bg-surface-50 rounded-2xl p-5 mb-5">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="email" dir="ltr" className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white text-left" value={editForm.email} onChange={e => setEditForm(f => ({ ...f, email: e.target.value }))} />
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">رقم الهاتف</label>
                  <div className="flex gap-2">
                    <select className="w-28 border border-surface-200 rounded-xl px-2 py-2.5 text-sm bg-white shrink-0" value={editForm.phoneCode} onChange={e => setEditForm(f => ({ ...f, phoneCode: e.target.value }))}>
                      {COUNTRY_CODES.map(c => <option key={c.code} value={c.code}>{c.country.split(' ')[0]} {c.code}</option>)}
                    </select>
                    <input type="tel" dir="ltr" className="flex-1 border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white text-left" value={editForm.phone} onChange={e => setEditForm(f => ({ ...f, phone: e.target.value }))} placeholder="1234567890" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">المحافظة (Provincia)</label>
                  <select className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white" value={editForm.province} onChange={e => setEditForm(f => ({ ...f, province: e.target.value }))}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[{ value: 'weak', label: 'ضعيف' }, { value: 'good', label: 'جيد' }, { value: 'very_good', label: 'جيد جداً' }, { value: 'native', label: 'أنا إيطالي' }].map(l => (
                      <button key={l.value} className={cn('py-2 rounded-xl border-2 text-xs font-medium', editForm.italianLevel === l.value ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-surface-200 text-surface-600 bg-white')}
            ...
            <div className="bg-surface-50 rounded-2xl p-5 mb-5">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="password" dir="ltr" className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white text-left" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} />
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">كلمة المرور الجديدة</label>
                  <input type="password" dir="ltr" className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white text-left" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="6 أحرف على الأقل" />
                </div>
                <div>
                  <label className="text-xs text-surface-500 mb-1 block">تأكيد كلمة المرور الجديدة</label>
                  <input type="password" dir="ltr" className="w-full border border-surface-200 rounded-xl px-3 py-2.5 text-sm bg-white text-left" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
                </div>
            ...
            {/* Save */}
            {saveMsg && (
              <div className={cn('rounded-xl p-3 mb-4 text-center text-sm font-medium', saveMsg.includes('✓') ? 'bg-success-50 text-success-600' : 'bg-danger-50 text-danger-600')}>
                {saveMsg}
              </div>
            )}
